...
def permute_qk(q, k):
    ...
    q = q.reshape(bsz, num_head, seq_len, head_dim // 2, 2).transpose(3, 4).reshape(bsz, num_head,
                                                                                    seq_len,
                                                                                    head_dim)
    k = k.reshape(bsz, num_head, seq_len, head_dim // 2, 2).transpose(3, 4).reshape(bsz, num_head,
                                                                                    seq_len,
                                                                                    head_dim)
...
def undo_permute_qk(q, k):
    ...
    # reference: https://github.com/young-geng/EasyLM/blob/981a2ed9630f44258a94b6f44dff2b7bd203ae8d/EasyLM/models/llama/convert_hf_to_easylm.py#L33
    bsz, num_head, seq_len, head_dim = q.shape
    q = q.reshape(bsz, num_head, seq_len, 2, head_dim // 2).transpose(3, 4).reshape(bsz, num_head,
                                                                                    seq_len,
                                                                                    head_dim)
    k = k.reshape(bsz, num_head, seq_len, 2, head_dim // 2).transpose(3, 4).reshape(bsz, num_head,
                                                                                    seq_len,
                                                                                    head_dim)
...
class TTTBase(nn.Module):
    ...
    def _init_ttt_lr_gate(self):
        ...
        self.learnable_ttt_lr_weight = nn.Parameter(
                torch.stack(
                        [torch.normal(0, 0.02, size=linear_weight_data.shape) for _ in
    ...
    def get_eta(self, X, mini_batch_step_offset, mini_batch_size):
        ...
        ttt_lr = torch.einsum("bnkc,hdc->bhnkd", X,
                              self.learnable_ttt_lr_weight) + self.learnable_ttt_lr_bias.reshape(
                1, -1, 1, 1, 1
        )
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            if use_dual_form:
                ...
                # [B,nh,1,f]
                b1_last = b1_init - torch.sum(last_eta_mini_batch * grad_l_wrt_Z1, dim=-2,
                                              keepdim=True)
                ...
                grad_W1 = torch.einsum("bhnk,bhkij->bhnij", torch.tril(ttt_lr_eta_mini_batch),
                                       grad_W1)
                grad_W1 = grad_W1 + params_dict["W1_grad"].unsqueeze(2)
                # [B, nh, K, f]
                grad_b1 = torch.einsum("bhnk,bhki->bhni", torch.tril(ttt_lr_eta_mini_batch),
                                       grad_l_wrt_Z1)
...
if __name__ == "__main__":
    ...
    class TTTLinearForSummary(TTTLinear):
        def forward(self, input_data):
            hidden_states = input_data["hidden_states"]
            position_ids = input_data["position_ids"]
            # mock 掉 cache
            return super().forward(hidden_states, position_ids, cache_params=None)


    from torchinfo import summary

    model = TTTLinearForSummary(num_heads=8, hidden_size=512, mini_batch_size=64, rope_theta=10000)
